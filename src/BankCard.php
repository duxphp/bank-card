...
class BankCard {
    ...
    public static function getBankList($rule = false) {
        if($rule) {
            return self::$bankRules;
        }else {
            return self::$bankList;
        }
    }

    public static function getBankNameList($rule = false) {
        if($rule) {
            return array_values(self::$bankRules);
        }else {
            return array_values(self::$bankList);
        }
    }
    ...
    public static function info($cardNum, $rule = false) {
        ...
        if (!$result->validated) {
            ...
            if ($rule && !self::$bankRules[$result->bank]) {
                $bankList = array(
                    'validated' => false
                );
            } else {
                $bankList = array(
                    ...
                    'bankName' => isset(self::$bankList[$result->bank]) ? self::$bankList[$result->bank] : '',   // 银行名称
